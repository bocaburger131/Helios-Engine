...
vi.mock('../../src/services/llmCategorizationService.js', () => ({
  LLMCategorizationService: class {
...
describe('RiskAnalysisService', () => {
  ...
  describe('analyzeRisk', () => {
    it('should analyze risk correctly for normal transactions', async () => {
      const transactions = [
      ...
      const result = await riskAnalysisService.analyzeRisk(transactions, 1000);
      expect(result).toBeDefined();
      expect(result.riskScore).toBeGreaterThanOrEqual(0);
      expect(result.riskScore).toBeLessThanOrEqual(850); // Veritas scale 0-850
      expect(['VERY_LOW', 'LOW', 'MEDIUM', 'HIGH']).toContain(result.riskLevel);
    });

    it('should indicate elevated risk for multiple NSF transactions', async () => {
      const transactions = [
        { date: '2024-01-01', description: 'NSF Fee', amount: -35.00 },
      ...
      const result = await riskAnalysisService.analyzeRisk(transactions, 0);
      expect(['MEDIUM', 'HIGH']).toContain(result.riskLevel);
      expect(result.riskScore).toBeLessThan(650);
    });

    it('should handle empty transactions array', async () => {
      const result = await riskAnalysisService.analyzeRisk([], 1000);
      expect(result).toBeDefined();
      expect(result.riskScore).toBeGreaterThanOrEqual(0);
      expect(result.riskScore).toBeLessThanOrEqual(850); // Veritas scale 0-850
      expect(['VERY_LOW', 'LOW', 'MEDIUM', 'HIGH']).toContain(result.riskLevel);
